...
public class Tour : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        for(int h = 0;h<interactables.Length;h++)
        {
            if(interactables[h].activeSelf)
            {
                canvas[5].SetActive(false);
                canvas[6].SetActive(false);
            }
        ...
        if(Input.GetKeyDown(KeyCode.Escape))
            {
                ...
                if(!vrControl.activeSelf)
                {
                    gyroNavOn();
                }
                else if(vrControl.activeSelf)
                {
                    controlStick.GetComponent<VRControlCam>().enabled = true;
                }
     ...
     public void stopVC()
    {
        ...
        {
            isCameraMove = false;
            controlStick.GetComponent<VRControlCam>().enabled = false;
